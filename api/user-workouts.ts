...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  if (req.method === 'GET') {
    const { data, error } = await supabaseAdmin
      .from('user_workouts')
      .select('*, library_workouts(*, workout_exercises(*, exercise_catalog(*)))')
      .eq('user_id', userId);
  ...
  if (req.method === 'POST') {
    ...
    const { data, error } = await supabaseAdmin
      .from('user_workouts')
      .upsert({ user_id: userId, workout_id: workoutId, is_favorite: isFavorite }, { onConflict: 'user_id,workout_id' })
      .select()
      .single();
